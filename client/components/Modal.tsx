    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          {/* dialog: responsive sizing, no scrollbars */}
          <motion.div initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 16, opacity: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className="relative w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl mx-4 sm:mx-6 bg-black p-4 sm:p-6 md:p-8 rounded-lg border border-[hsl(var(--border))] shadow-2xl max-h-[85vh] sm:max-h-[90vh] overflow-hidden">
